...
//                 <input type="text" placeholder="Slot Title" value={title} onChange={e => setTitle(e.target.value)} className="p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500" />
//                 <input type="date" value={date} onChange={e => setDate(e.target.value)} className="p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500" />
...
//               {crons.map((cron) => (
//                 <div key={cron._id} className="relative p-6 bg-gradient-to-br from-indigo-50 via-white to-blue-100 rounded-2xl shadow-lg border border-indigo-100 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
...
//                   <span className={`inline-block px-3 py-1 text-sm font-medium rounded-full mb-4 ${cron.reminderSent ? 'bg-green-100 text-green-700 border border-green-300' : 'bg-red-100 text-red-700 border border-red-300'}`}>
